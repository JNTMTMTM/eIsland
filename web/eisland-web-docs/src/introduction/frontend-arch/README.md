...
The eIsland frontend is built on Electron's multi-process architecture with a strict separation between the **Main Process** (Node.js), the **Preload Bridge** (Context Bridge), and the **Renderer Process** (Chromium). The core of the application is a state machine that manages **23 distinct states** controlling the island's appearance, expansion, and interaction behavior.
