> **Release Date:** *`2026-07-18`* **GitHub Repository:** *`https://github.com/JNTMTMTM/eIsland`*

**V26.6.5 新增 URL 收藏管理、邮件收发、备忘录编辑、本地文件搜索、待办清单、倒计时与剪贴板历史共 7 个全新功能模块，同步扩展 Gitee 与 KOOK 第三方登录支持，并带来相册浏览与多项交互体验优化。**

*V26.6.5 introduces 7 brand-new feature modules — URL Favorites management, Mail sending/receiving, Memo editing, Local File Search, Todo list, Countdown timer, and Clipboard History — along with expanded Gitee and KOOK third-party login support, and brings an album viewer with multiple interaction experience improvements.*

## 新功能

- 新增 URL 收藏管理模块，支持收藏的添加、编辑、删除与搜索，提供拖拽排序与导入导出功能。
- 新增邮件模块，支持多账户收件箱管理，提供邮件列表浏览与邮件详情阅读功能。
- 新增备忘录模块，支持备忘录的创建、编辑、删除与搜索，提供侧边栏导航与空状态引导。
- 新增本地文件搜索模块，支持本地文件检索与配置管理，提供搜索结果展示与过渡动画效果。
- 新增待办清单模块，支持任务的创建、编辑与管理。
- 新增倒计时模块，支持倒计时事件的创建与管理，提供日历视图与卡片展示。
- 新增剪贴板历史模块，支持剪贴板记录的查看与管理，提供展开模式与详情展示。
- 新增相册浏览模块，支持相册图片的网格展示、元数据查看与批量选择操作。
- 新增 Gitee 第三方登录支持，可在登录界面与用户设置中选择 Gitee 账号授权登录。
- 新增 KOOK 第三方登录支持，可在登录界面与用户设置中选择 KOOK 账号授权登录。
- 新增 微信 第三方登录支持，可在登录界面与用户设置中选择 KOOK 账号授权登录。
- 新增 微软 第三方登录支持，可在登录界面与用户设置中选择 KOOK 账号授权登录。

## 体验优化

- 优化本地文件搜索配置面板的过渡动画与布局效果，交互更流畅。
- 优化相册组件的文件处理与数据管理，操作响应更灵敏。
- 优化备忘录编辑器的界面布局与过渡效果，编辑体验更自然。
- 优化登录面板宽度适配，界面展示更协调。

## 问题修复

- 修复登录面板宽度不足导致的布局问题。
- 修复相册浏览组件中背景媒体预览地址的导入路径错误。
- 修复邮件模块初始化时的竞态条件，避免快速切换账户时旧请求覆盖新账户数据。

## 文档更新

- 自动更新变更日志，记录 PR #149 至 #158 的全部提交内容。

感谢大家持续反馈与支持。若你在升级后发现新问题，欢迎继续反馈，我们会尽快跟进。

## New Features

- Added URL Favorites management module with add, edit, delete, and search capabilities, supporting drag-and-drop sorting and import/export functionality.
- Added Mail module with multi-account inbox management, supporting mail list browsing and mail detail reading.
- Added Memo module with create, edit, delete, and search capabilities, providing sidebar navigation and empty state guidance.
- Added Local File Search module with local file retrieval and configuration management, providing search results display and transition animation effects.
- Added Todo module with task creation, editing, and management capabilities.
- Added Countdown module with countdown event creation and management, providing calendar view and card display.
- Added Clipboard History module with clipboard record viewing and management, providing expanded mode and detail display.
- Added Album Viewer module with grid display, metadata viewing, and batch selection operations for album images.
- Added Gitee third-party login support, available on the login screen and in user settings for Gitee account authorization.
- Added KOOK third-party login support, available on the login screen and in user settings for KOOK account authorization.
- Added WeChat third-party login support, available on the login screen and in user settings for WeChat account authorization.
- Added Microsoft third-party login support, available on the login screen and in user settings for Microsoft account authorization.

## Improvements

- Optimized Local File Search configuration panel transition animations and layout effects for smoother interactions.
- Optimized album component file handling and data management for more responsive operations.
- Optimized Memo editor interface layout and transition effects for a more natural editing experience.
- Optimized login panel width adaptation for a more balanced interface display.

## Bug Fixes

- Fixed layout issues caused by insufficient login panel width.
- Fixed incorrect import path for background media preview URL in the album viewer component.
- Fixed race condition during mail module initialization that could cause old requests to overwrite new account data when switching accounts quickly.

## Documentation

- Automatically updated the changelog with all commit records from PR #149 through #158.

Thank you for your continued feedback and support. If you encounter new issues after upgrading, please continue to report them — we will follow up as soon as possible.